...
('Ubuntu (Systemd VPS)', 'ubuntu-systemd', 'os', 'Ubuntu с рабочим systemd (PID 1). Позволяет использовать systemctl и запускать службы (подходит для установки сложных скриптов).', 'jrei/systemd-ubuntu', '22.04', 'ubuntu', '{"ports":[],"env":[],"volumes":[{"host":"/sys/fs/cgroup","container":"/sys/fs/cgroup:ro"}],"tmpfs":["/tmp","/run","/run/lock"],"cmd":"","cpu":"1","ram":"512m","restart":"unless-stopped","privileged":true}', 2),
